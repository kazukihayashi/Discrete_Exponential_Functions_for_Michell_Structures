...
def Truss2D(pts_all,lines,fix,load,force,n_color,label,n_size,l_width,name):
    ...
    for i in range(pts_all.shape[0]):
        p[i] = ax.scatter(pts_all[i,~fix,0],pts_all[i,~fix,1],color=n_color[i],s=n_size,marker="o",zorder=i)
    ...
    ax.quiver(ptx,pty,load[loaded,0],load[loaded,1],color="red",scale=5)

    if label is not None:
        line_ends = np.array([pts_all[0,lines[j,:]] for j in range(len(lines))])
        divnorm=colors.TwoSlopeNorm(vcenter=0.0)
        lc1 = LineCollection(line_ends,linestyle=':',linewidth=l_width,cmap=cm,colors=(0.0,0.0,0.0,0.2),norm=divnorm,zorder=-10)
    ...
    lc2 = LineCollection(line_ends,linewidth=l_width,cmap=cm,colors=(0.0,0.0,0.0,0.2),norm=divnorm,zorder=-5)
    lc2.set_array(force)
